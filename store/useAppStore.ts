...
import { CartItem, UserLocation, MenuItem, Voucher, CheckoutPreferences, TipOption, Order } from '@/types';
import { defaultLocation, savedAddresses, vouchers, deliveryOptions, tipOptions, orders } from '@/mocks/data';
...
interface AppState {
  ...
  updateCartItemQuantity: (itemId: string, quantity: number, option: string) => Promise<void>;
  ...
  persist(
    (set, get) => ({
      ...
      getCartTotal: () => {
        ...
        return cart.reduce((total, item) => {
          ...
          const optionsTotal = item.options
            ? item.options.reduce((sum, option) => sum + (option.price || 0), 0) * item.quantity
            : 0;
      ...
      placeOrder: (orderData) => {
        ...
        const orderId = `${Math.floor(10000 + Math.random() * 90000)}-${Math.floor(100000000 + Math.random() * 900000000)}`;
        ...
        const newOrder: Order = {
          ...
          user: {
            id: 'hungthinhh2003',
            name: 'hungthinhh2003',
            phoneNumber: checkoutPreferences.selectedAddress.id === '1' ? '0944034769' : '0123456789',
            createdAt: new Date(),
            isGroupLeader: true
          },
          restaurant: {
          ...
          deliveryAddress: {
            ...
            contactPhone: checkoutPreferences.selectedAddress.id === '1' ? '0944034769' : '0123456789'
          },
          ...
          subtotal: subtotal,
          discount: discount,
          total: subtotal + (deliveryOption?.price || 15000) + Math.round(subtotal * 0.05) - discount + checkoutPreferences.tipAmount,
          status: 'COMPLETED',
          paymentMethod: checkoutPreferences.selectedPaymentMethod === 'cash' ? 'Cash' : 'ShopeePay',
          paymentStatus: 'PAID',
